	   ...
	   //this message is solely for debugging purposes. You can delete or or comment it out when implemented on your code
		...
		//test if it is an image. Important to prevent malicious software from being uploaded to your server
			...
			//After all those checks, this is where the image is uploaded to server
				...
				$directory = dirname(__FILE__) . "/uploadedimages/" ;
				...
				if(move_uploaded_file($_FILES["Image"]["tmp_name"][$key], $directory . $imageName ) === TRUE){
					...
					$dbDirectory = "uploadedimages/" . $imageName;
...
// imagedb.setimageorder(imageName);
// imagedb.setimagecategory(imageName);
// imagedb.setimagedate(imageName);

  $imagedetails->setimageuploaddate(date("Y/m/d"));
    $imagedetails->setImagePositione($key);
    $imagedetails->setImagePath($dbDirectory);
    $imagedetails->setImageName($imageName);
 

 $Id=0;
   if($Id>0)
   {
    $result=$imagedetails->Update($Id); 
